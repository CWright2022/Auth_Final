...
Remove-Item -ErrorAction SilentlyContinue $env:TEMP\test09_Weak_temp.txt
$startTime = Get-Date
Push-Location "$PSScriptRoot\..\john"
cmd /c "john.exe --format=raw-sha256 --wordlist=..\rockyou.txt --fork=8 ..\hashes_weak.txt" | Tee-Object -FilePath $env:TEMP\test09_Weak_temp.txt
Pop-Location
Get-Content $env:TEMP\test09_Weak_temp.txt | Add-Content $OutputFile
$recoveredLine = Select-String -Path $env:TEMP\test09_Weak_temp.txt -Pattern "^Recovered" | Select-Object -Last 1
if ($recoveredLine) {
...
Remove-Item -ErrorAction SilentlyContinue hashcat.potfile
Remove-Item -ErrorAction SilentlyContinue $env:TEMP\test09_Medium_temp.txt
$startTime = Get-Date
Push-Location "$PSScriptRoot\..\john"
cmd /c "john.exe --format=raw-sha256 --wordlist=..\rockyou.txt --fork=8 ..\hashes_medium.txt" | Tee-Object -FilePath $env:TEMP\test09_Medium_temp.txt
Pop-Location
Get-Content $env:TEMP\test09_Medium_temp.txt | Add-Content $OutputFile
$recoveredLine = Select-String -Path $env:TEMP\test09_Medium_temp.txt -Pattern "^Recovered" | Select-Object -Last 1
if ($recoveredLine) {
    $lineText = $recoveredLine.ToString().Trim()
...
Remove-Item -ErrorAction SilentlyContinue hashcat.potfile
Remove-Item -ErrorAction SilentlyContinue $env:TEMP\test09_Strong_temp.txt
$startTime = Get-Date
Push-Location "$PSScriptRoot\..\john"
cmd /c "john.exe --format=raw-sha256 --wordlist=..\rockyou.txt --fork=8 ..\hashes_strong.txt" | Tee-Object -FilePath $env:TEMP\test09_Strong_temp.txt
Pop-Location
Get-Content $env:TEMP\test09_Strong_temp.txt | Add-Content $OutputFile
$recoveredLine = Select-String -Path $env:TEMP\test09_Strong_temp.txt -Pattern "^Recovered" | Select-Object -Last 1
if ($recoveredLine) {
    $lineText = $recoveredLine.ToString().Trim()
