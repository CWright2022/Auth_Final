...
START=$(date +%s)
hashcat -m 1400 -a 0 -O --force --opencl-device-types 1  -n 3 hashes_weak.txt rockyou.txt
END=$(date +%s)
...
START=$(date +%s)
hashcat -m 1400 -a 0 -O --force --opencl-device-types 1  -n 3 hashes_medium.txt rockyou.txt
END=$(date +%s)
...
START=$(date +%s)
hashcat -m 1400 -a 0 -O --force --opencl-device-types 1  -n 3 hashes_strong.txt rockyou.txt
END=$(date +%s)
