...
send_webhook ":rocket: Starting Test 15: John CPU (3 threads), Dictionary mode"

echo "Running Test 15: John CPU (3 threads), Dictionary mode"
OUTFILE="test15_results.txt"

echo "[*] Cracking WEAK passwords..." | tee -a "$OUTFILE"
send_webhook "[*] Cracking WEAK passwords..."
START=$(date +%s)
john --format=raw-sha256 --wordlist=rockyou.txt --fork=3 hashes_weak.txt
END=$(date +%s)
...
START=$(date +%s)
john --format=raw-sha256 --wordlist=rockyou.txt --fork=3 hashes_medium.txt
END=$(date +%s)
...
START=$(date +%s)
john --format=raw-sha256 --wordlist=rockyou.txt --fork=3 hashes_strong.txt
END=$(date +%s)
echo "STRONG time: $((END - START)) seconds" | tee -a "$OUTFILE"
send_webhook "STRONG time: $((END - START)) seconds"


send_webhook ":white_check_mark: Test 15 complete! Check output file for time breakdown."