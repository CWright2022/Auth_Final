...
cmd /c "john.exe --format=raw-sha256 --wordlist=rockyou.txt --fork=4 hashes_weak.txt" | Tee-Object -FilePath $OutputFile -Append
$endTime = Get-Date
...
$startTime = Get-Date
Remove-Item -ErrorAction SilentlyContinue john.pot
cmd /c "john.exe --format=raw-sha256 --wordlist=rockyou.txt --fork=4 hashes_medium.txt" | Tee-Object -FilePath $OutputFile -Append
$endTime = Get-Date
$elapsed = ($endTime - $startTime).TotalSeconds
...
$startTime = Get-Date
Remove-Item -ErrorAction SilentlyContinue john.pot
cmd /c "john.exe --format=raw-sha256 --wordlist=rockyou.txt --fork=4 hashes_strong.txt" | Tee-Object -FilePath $OutputFile -Append
$endTime = Get-Date
$elapsed = ($endTime - $startTime).TotalSeconds
