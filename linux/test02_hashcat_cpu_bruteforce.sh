...
send_webhook ":rocket: Starting Test 02: Hashcat CPU (1 threads), Bruteforce mode"

echo "Running Test 02: Hashcat CPU (1 threads), Bruteforce mode"
OUTFILE="test02_results.txt"

echo "[*] Cracking WEAK passwords..." | tee -a "$OUTFILE"
send_webhook "[*] Cracking WEAK passwords..."
START=$(date +%s)
hashcat -m 1400 -a 3 -O --force --opencl-device-types 1  --cpu-affinity=1 hashes_weak.txt '?a?a?a?a?a?a'
END=$(date +%s)
...
START=$(date +%s)
hashcat -m 1400 -a 3 -O --force --opencl-device-types 1  --cpu-affinity=1 hashes_medium.txt '?a?a?a?a?a?a'
END=$(date +%s)
...
START=$(date +%s)
hashcat -m 1400 -a 3 -O --force --opencl-device-types 1  --cpu-affinity=1 hashes_strong.txt '?a?a?a?a?a?a'
END=$(date +%s)
echo "STRONG time: $((END - START)) seconds" | tee -a "$OUTFILE"
send_webhook "STRONG time: $((END - START)) seconds"


send_webhook ":white_check_mark: Test 02 complete! Check output file for time breakdown."