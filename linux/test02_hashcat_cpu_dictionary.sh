...
OUTFILE="results/test03_results.txt"

echo "[*] Cracking WEAK passwords..." | tee -a "$OUTFILE"
send_webhook "[*] Cracking WEAK passwords..."
START=$(date +%s)
../hashcat/hashcat -m 1400 -a 0 -O --force --opencl-device-types 1  --cpu-affinity=4 hashes/hashes_weak.txt ../realhuman_phill.txt
END=$(date +%s)
...
START=$(date +%s)
../hashcat/hashcat -m 1400 -a 0 -O --force --opencl-device-types 1  --cpu-affinity=4 hashes/hashes_medium.txt ../realhuman_phill.txt
END=$(date +%s)
...
START=$(date +%s)
../hashcat/hashcat -m 1400 -a 0 -O --force --opencl-device-types 1  --cpu-affinity=4 hashes/hashes_strong.txt ../realhuman_phill.txt
END=$(date +%s)
