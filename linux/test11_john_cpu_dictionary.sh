...
send_webhook ":rocket: Starting Test 11: John CPU (1 threads), Dictionary mode"

echo "Running Test 11: John CPU (1 threads), Dictionary mode"
OUTFILE="test11_results.txt"

echo "[*] Cracking WEAK passwords..." | tee -a "$OUTFILE"
send_webhook "[*] Cracking WEAK passwords..."
START=$(date +%s)
john --format=raw-sha256 --wordlist=rockyou.txt --fork=1 hashes_weak.txt
END=$(date +%s)
...
START=$(date +%s)
john --format=raw-sha256 --wordlist=rockyou.txt --fork=1 hashes_medium.txt
END=$(date +%s)
...
START=$(date +%s)
john --format=raw-sha256 --wordlist=rockyou.txt --fork=1 hashes_strong.txt
END=$(date +%s)
echo "STRONG time: $((END - START)) seconds" | tee -a "$OUTFILE"
send_webhook "STRONG time: $((END - START)) seconds"


send_webhook ":white_check_mark: Test 11 complete! Check output file for time breakdown."